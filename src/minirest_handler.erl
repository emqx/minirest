        ...
        Params ->
            case erlang:apply(Mod, Fun, [Bindings, Params]) of
                {file, Headers, {sendfile, _, _, _} = SendFile} ->
                    do_cow_reply(200, Headers, SendFile, Req);
...
jsonify({ok, Headers, Response}, Req) ->
    do_cow_reply(200, Headers, Response, Req);
...
jsonify(Code, Headers, Response, Req) ->
    try json_encode(Response) of
        Json ->
            do_cow_reply(Code, Headers, Json, Req)
    catch
...
reply(Code, Text, Req) ->
    do_cow_reply(Code, #{<<"content-type">> => <<"text/plain">>}, Text, Req).

do_cow_reply(Code, Headers, Body, Req) ->
    minirest:do_cow_reply(Code, Headers, Body, Req).
